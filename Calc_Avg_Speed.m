...
idx_speed(1,1) = find((speed >= (belt_speeds(1)-0.025)), 1, 'first') + 1.5/(time(2)-time(1)); % 1.5 second following the first time speed increases above 1% below expected speed, here 1/(time(2)-time(1)) serves as sampling frequency
idx_speed(1,2) = idx_speed(1,1) + 27/(time(2)-time(1)); % 27 seconds after the treadmill gets up to expected speed, here 1/time(2) serves as sampling frequency
for j = 2:size(belt_speeds,2)
    idx_speed(j,1:2) = round(idx_speed(j-1,1:2) + 30/(time(2)-time(1))); % speeds changed every 30s, here 1/(time(2)-time(1)) serves as sampling frequency
end

% calculate average speed over period & times when corresponding speed
% starts and ends
for j = 1:size(idx_speed,1)
    if idx_speed(j,2) <= size(speed,1) % have data for entire speed
        avg_speed(j,1) = mean(speed(idx_speed(j,1):idx_speed(j,2)));
        start(j,1) = time(round(idx_speed(j,1)));
        finish(j,1) = time(round(idx_speed(j,2)));
    else % if there's no data for that speed, don't include it in the index
        idx_speed(j,1:2) = nan(1,2);
    end 
end
...
if strcmp(plt(1),'Y')==1 || strcmp(plt(1),'y')==1
    ...
    plot(time,speed)
    hold on;
    plot(time(round(rmmissing(idx_speed(:,1)))), speed(round(rmmissing(idx_speed(:,1)))),'r>')
    plot(time(round(rmmissing(idx_speed(:,2)))), speed(round(rmmissing(idx_speed(:,2)))),'r<')
end